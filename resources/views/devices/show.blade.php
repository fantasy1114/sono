...
<section class="users-list-wrapper section edit__index__page @if(Auth::user()->is_superuser == 0) {{'d-none'}} @endif">
    ...
    <div class="row valign-wrapper index__solid__border">
        <div class="col-12">
            <a href="{{ route('devices.device.index') }}" class="btn-floating btn waves-effect waves-light show__back__btn ml-md-2">
              <i class="material-icons" title="{{ trans('devicess.create') }}">arrow_back</i>
            </a>
            <h5 class="white-text d-md-inline align-middle indexpage__title__size show__index__name ml-md-2">{{ isset($title) ? $title : trans('devices.model_plural') }}</h5>
            <a href="{{ route('devices.device.create') }}" class="btn waves-effect waves-light d-flex align-items-center float-right show__add__btn">
              <div class="d-inline">{{ trans('locale.add') }}</div><i class="material-icons" title="{{ trans('devices.create') }}">add</i>
            </a>
        </div>
    </div>
        ...
        <div class="card-content row">
          ...
          <div class="col-lg-6 col-12 card-width">
            <div class="input-field">
                ...
                <div class="form-group">
                    <input type="text" value="{{ date('d.m.Y H:i', strtotime(str_replace('/', '-', $device->Created_At))) }}" class="form-control pl-0 placeholder_fonts validate" />
                </div>
            </div>
          </div>
          <div class="col-lg-6 col-12 card-width">
            <div class="input-field">
                ...
                <div class="form-group">
                    <input type="text" value="{{ date('d.m.Y H:i', strtotime(str_replace('/', '-', $device->Updated_At))) }}" class="form-control pl-0 placeholder_fonts validate" />
                </div>
            </div>
          </div>
