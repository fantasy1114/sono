...
<section class="users-list-wrapper section @if(Auth::user()->is_superuser == 0) {{'d-none'}} @endif">
    ...
    <div class="col-12 valign-wrapper index__solid__border">
        <div class="col s9 m9 l9 left-align">
            <h5 class="white-text indexpage__title__size">{{ trans('devices.model_plural') }}</h5>
            <p class="indexpage__user__explain">{!! auth()->user()->renderOrgName() !!}</p>
        </div>
        <div class="col s3 m3 l3 right-align pr-0">
            <a href="{{ route('devices.device.create') }}" class="btn waves-effect waves-light d-flex align-items-center float-right indexpage__add__btn">
              ...
              <div class="card">
                <div class="py-3">
                      ...
                      <table id="page-length-option" class="display">
                        <thead>
                          <tr>
                            <th class="px-3">{{ trans('devices.Tracker_Code') }}</th>
                        ...
                        <tbody>
                          @foreach($devices as $device)
                            <tr>
                              <td class="px-3">{{ $device->Tracker_Code }}</td>
                              <td>{{ optional($device->Worksite)->Worksite_Name }}</td>
                              <td><span class="{{ ($device->Is_Active) ? 'statusactive' : 'statusinactive' }}">{{ ($device->Is_Active) ? 'Active' : 'Inactive' }}</span></td>

                              <form method="POST" action="{!! route('devices.device.destroy', $device->Device_ID) !!}" accept-charset="UTF-8">
                                <td class="">
                                  <a class="btn-flat" href="{{ route('devices.device.show', $device->Device_ID ) }}"><i class=""><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  ...
                                  <a class="btn-flat" href="{{ route('devices.device.edit', $device->Device_ID ) }}"><i class=""><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
