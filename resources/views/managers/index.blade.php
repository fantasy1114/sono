...
<section class="users-list-wrapper section @if(Auth::user()->is_superuser == 0) {{'d-none'}} @endif">
    ...
    <div class="col-12 valign-wrapper index__solid__border">
        <div class="col s9 m9 l9 left-align">
            <h5 class="white-text indexpage__title__size">{{ trans('managers.model_plural') }}</h5>
        ...
        <div class="col s3 m3 l3 right-align pr-0">
            <a href="{{ route('managers.manager.create') }}" class="btn waves-effect waves-light d-flex align-items-center float-right indexpage__add__btn">
            ...
            <div class="card rounded mobile_indexpage">
              <div class="py-3">
                    ...
                    <table id="page-length-option" class="display">
                      <thead>
                        <tr>
                          <th class="px-3">{{ trans('managers.name') }}</th>
                      ...
                      <tbody>
                        @foreach($managers as $manager)
                          <tr>
                            <td class="px-3">{{ $manager->name }}</td>
                            <td>{{ $manager->email }}</td>
                            <td>{{ optional($manager->Organisation)->Organisation_Name }}</td>
                            <td>{{ ($manager->is_superuser) ? trans('locale.Yes') : trans('locale.No') }}</td>
                            <td><span class="{{ ($manager->is_active) ? 'statusactive' : 'statusinactive' }}">{{ ($manager->is_active) ? 'Active' : 'Inactive' }}</span></td>

                            <form method="POST" action="{!! route('managers.manager.destroy', $manager->id) !!}" accept-charset="UTF-8">
                              <td class="">
                                <a class="btn-flat" href="{{ route('managers.manager.show', $manager->id ) }}"><i class=""><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a class="btn-flat" href="{{ route('managers.manager.edit', $manager->id ) }}"><i class=""><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
