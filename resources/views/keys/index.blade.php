...
<section class="users-list-wrapper section">
    ...
    <div class="col-12 valign-wrapper index__solid__border">
        <div class="col s9 m9 l9 left-align">
            <h5 class="white-text indexpage__title__size">{{ trans('keys.model_plural') }}</h5>
            <p class="indexpage__user__explain">{!! auth()->user()->renderOrgName() !!}</p>
        </div>
        <div class="col s3 m3 l3 right-align pr-0">
            <a href="{{ route('keys.key.create') }}" class="btn waves-effect waves-light d-flex align-items-center float-right indexpage__add__btn">
            ...
            <div class="card">
              <div class="py-3">
                    ...
                    <table id="page-length-option" class="display">
                      <thead>
                        <tr>
                          <th class="px-3">{{ trans('keys.Key_Name') }}</th>
                      ...
                      <tbody>
                        @foreach($keys as $key)
                          <tr>
                            <td class="px-3">{{ $key->Key_Name }}</td>
                            <td>{{ optional($key->Employee)->Employee_Name }}</td>
                            <td><span class="{{ ($key->Is_Active) ? 'statusactive' : 'statusinactive' }}">{{ ($key->Is_Active) ? 'Active' : 'Inactive' }}</span></td>

                            <form method="POST" action="{!! route('keys.key.destroy', $key->Key_ID) !!}" accept-charset="UTF-8">
                              <td class="">
                                <a class="btn-flat" href="{{ route('keys.key.show', $key->Key_ID ) }}"><i class=""><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a class="btn-flat" href="{{ route('keys.key.edit', $key->Key_ID ) }}"><i class=""><svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
