...
<section class="users-list-wrapper section edit__index__page">
    ...
    <div class="col-12 valign-wrapper index__solid__border">
        <div class="left-align">
            <h5 class="white-text indexpage__title__size">{{ isset($title) ? $title : trans('employees.model_plural') }}</h5>
        </div>
        <!-- "Go Up" button -->
        <div class="col s3 m3 l3 right-align">
            <a href="{{ route('employees.employee.create') }}" class="btn waves-effect waves-light d-flex align-items-center float-right edit__add__btn">
            ...
            <div class="card-panel mb-6 rounded">
                ...
                <div class="row edit__btns__group">
                    <div class="col-sm-3 col-xl-2">
                        <input class="btn btn-primary editpage__update__btn" type="submit" value="{{ trans('employees.update') }}">
                        </form>
                    </div>
                    <div class="col-sm-9 col-xl-10">
                        <div class="float-left ml-sm-5">
                            <a href="{{ route('employees.employee.index') }}" class="btn waves-effect waves-light darken-2 editpage__cancel__btn">{{ trans('locale.cancels') }}</a>
                        </div>
                        <div class="float-right">
                            <form method="POST" action="{!! route('employees.employee.destroy', $employee->Employee_ID) !!}" accept-charset="UTF-8">
                                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                <input name="_method" value="DELETE" type="hidden">
                                <input class="btn btn-primary editpage__delete__btn" onclick="return confirm(&quot;{{ trans('employees.confirm_delete') }}&quot;)" type="submit" value="{{ trans('locale.deletes') }}">
                            </form>
                        </div>
                    </div>
